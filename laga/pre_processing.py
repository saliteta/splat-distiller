# Copied from LangSplat (https://github.com/minghanqin/LangSplat.git)
...
def create(image_list, data_list, save_folder, norm_clip_features):
    ...
    for i, img in tqdm(enumerate(image_list), desc="Embedding images", leave=False, total=len(image_list)):
        timer += 1
        try:
            img_embed, seg_map = _embed_clip_sam_tiles(img.unsqueeze(0), sam_encoder, norm = norm_clip_features)
        except:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sam_ckpt_path', type=str, default="/datasets/sam_ckpt/sam_vit_h_4b8939.pth")
    ...
    save_folder = os.path.join(dataset_path, 'language_features_test')

    # save_folder = os.path.join(dataset_path, 'language_features_langsplat')
    # save_folder = os.path.join(dataset_path, 'language_features_test_3dovs')

    os.makedirs(save_folder, exist_ok=True)
    create(imgs, data_list, save_folder, norm_clip_features=True)