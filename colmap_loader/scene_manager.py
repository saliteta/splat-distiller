...
class SceneManager:
    ...
    def _load_images_txt(self, input_file):
        ...
        with open(input_file, 'r') as f:
            for line in f:
                ...
                # This is an image description line
                image_id = int(data[0])
                image = Image(data[-1], int(data[-2]),
                              Quaternion(np.array(list(map(float, data[1:5])))),
                              np.array(list(map(float, data[5:8]))))
                
                # Set empty point data since this file doesn't contain point observations
                image.points2D = np.empty((0, 2))
        ...
        if input_file is None:
            ...
            if os.path.exists(input_file):
                ...
                if os.path.exists(input_file):
                    ...
                    input_file = self.folder + 'points3D.ply'
                    if os.path.exists(input_file):
                        self._load_points3D_ply(input_file)
                    else:
                        raise IOError('no points3D file found (tried .bin, .txt, .ply)')
    ...
    def _load_points3D_ply(self, input_file):
        """Load points from binary PLY file. Note: PLY files don't contain tracking info."""
        self.points3D = []
        self.point3D_ids = []
        self.point3D_colors = []
        self.point3D_id_to_point3D_idx = dict()
        ...
        with open(input_file, 'rb') as f:
            ...
            # Extract vertex count and format
            vertex_count = 0
            ...
            # PLY format: x y z r g b nx ny nz (3 doubles + 3 bytes + 3 doubles)
            endian = '<' if format_type == 'binary_little_endian' else '>'
            vertex_struct = struct.Struct(f'{endian}3d3B3d')  # 3 doubles (xyz) + 3 bytes (rgb) + 3 doubles (normals)
            
            # Read vertex data
            for i in range(vertex_count):
                try:
                    data = vertex_struct.unpack(f.read(vertex_struct.size))
                    x, y, z, r, g, b, nx, ny, nz = data
                    
                    self.points3D.append([x, y, z])
                    self.point3D_colors.append([r, g, b])
                    
                    point3D_id = np.uint64(i)
                    self.point3D_ids.append(point3D_id)
                    self.point3D_id_to_point3D_idx[point3D_id] = i
                    self.point3D_errors.append(0.0)
        ...
        print(f'Loaded {len(self.points3D)} points from binary PLY file (no tracking info available)')
