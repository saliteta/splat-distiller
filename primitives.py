...
class GaussianPrimitive(Primitive):
    ...
    def filtering(self, trainloader: torch.utils.data.DataLoader, means: torch.Tensor, device: torch.device, threshold: float = 0.2, args: Optional[dict] = None) -> torch.Tensor:
        """
...
class BetaSplatPrimitive(GaussianPrimitive):
    ...
    def _from_dbs_ckpt(self, ckpt_path: Union[str, Path], feature_path: Optional[Union[str, Path]] = None) -> None:
        """
        Load a BetaSplat primitive from a checkpoint.
        """
        ckpt, _ = torch.load(ckpt_path, map_location="cuda")
        sh_degree, means, sh0, shN, sb_params, scaling, rotation, opacity, beta, _, _,  = ckpt
        self._geometry = {
            "means": means,
            "opacities": torch.sigmoid(opacity.squeeze(-1)),
            "scales": torch.exp(scaling),
    ...
    def _from_ply(self, file_path: Union[str, Path], feature_path: Optional[Union[str, Path]] = None) -> None:
        """
        ...
        self._source_data = {  
            ...
            "sh0": torch.tensor(sh0, dtype=torch.float, device="cuda").transpose(1,2).cpu(),
            "sb_params": torch.tensor(sb_params, dtype=torch.float, device="cuda").transpose(1,2).cpu(),
        ...
        sb_params = torch.tensor(sb_params, dtype=torch.float, device="cuda").transpose(1,2)
    ...
    def _load_feature_from_ply(self, ckpt_path: Union[str, Path], feature_path: Union[str, Path]| None = None) -> None:
        """
        Load features from a ply file.
        """
        if feature_path is not None:
            self._feature = torch.load(feature_path, map_location="cuda")
        elif os.path.exists(Path(ckpt_path).parent / (Path(ckpt_path).stem + "_features.pt")):
            possible_feature_path = Path(ckpt_path).parent / (Path(ckpt_path).stem + "_features.pt")
    ...
    def _from_ours_ckpt(self, ckpt_path: Union[str, Path], feature_path: Optional[Union[str, Path]] = None, transformed: bool = False) -> None:
        """
        Load a BetaSplat primitive from a checkpoint.
        """
    ...
    def from_file(self, file_path: Union[str, Path], feature_path: Optional[Union[str, Path]] = None, transformed: bool = False) -> None:
        """
        Load a BetaSplat primitive from a file.
        """
...
if __name__ == "__main__":
    primitive = BetaSplatPrimitive()
    primitive.from_file("/media/bxiong/c6deb427-f841-4fb3-8707-2d0593655c63/models/results_metrics/waldo_kitchen/DBS/point_cloud/iteration_60000/point_cloud.ply")
