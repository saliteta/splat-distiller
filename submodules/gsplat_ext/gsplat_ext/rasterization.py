...
from bsplat.cuda._wrapper import fully_fused_projection as fully_fused_projection_beta
from bsplat.cuda._wrapper import isect_tiles as isect_tiles_beta
from bsplat.cuda._wrapper import isect_offset_encode as isect_offset_encode_beta
...
def inverse_rasterization_bsplats(
    means: Tensor,  # [N, 3]
    quats: Tensor,  # [N, 4]
    scales: Tensor,  # [N, 3]
    opacities: Tensor,  # [N]
    ...
    *kwargs, 
    ...
    # Project Gaussians to betas. Directly pass in {quats, scales} is faster than precomputing covars.
    proj_results = fully_fused_projection_beta(
        means,
        covars,
        quats,
        scales,
    ...
    if packed:
        ...
        print("we only support packed is true")
    ...
    tile_width = math.ceil(width / float(tile_size))
    tile_height = math.ceil(height / float(tile_size))
    tiles_per_gauss, isect_ids, flatten_ids = isect_tiles_beta(
        means2d,
        radii,
        depths,
        tile_size,
    ...
    # print("rank", world_rank, "Before isect_offset_encode")
    isect_offsets = isect_offset_encode_beta(isect_ids, C, tile_width, tile_height)
    ...
    if input_image.shape[-1] > channel_chunk:
        ...
        for i in range(n_chunks):
            ...
            feature_gaussian_, feature_weight_ = rasterize_reverse_fwd_betas(
                means2d,
                conics,
                opacities,
                betas,
        ...
        feature_gaussian, feature_weight = rasterize_reverse_fwd_betas(
            means2d,
            conics,
            opacities,
            betas,
    ...
    return feature_gaussian, feature_weight, primitive_ids
