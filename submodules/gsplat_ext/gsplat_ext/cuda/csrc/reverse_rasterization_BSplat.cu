...
__global__ void reverse_rasterize_to_bsplat_kernel(
    const uint32_t C,
    const uint32_t N,
...
void launch_reverse_rasterize_to_bsplats_kernel(
    // Gaussian parameters
        const at::Tensor &means2d,         // [C, N, 2] or [nnz, 2]
        const at::Tensor &conics,  // [C, N, 3, 3] or [nnz, 3, 3]
        const at::Tensor &opacities,  // [C, N, 3, 3] or [nnz, 3, 3]
        const at::Tensor &betas,  // [C, N, 3, 3] or [nnz, 3, 3]
    ...
    int64_t shmem_size = 
    tile_size * tile_size * (sizeof(int32_t) +  sizeof(vec3) + sizeof(vec4)); // xy_opac_batch, uMs, vMs, wMs

    // Allow large shared memory
        if (cudaFuncSetAttribute(
                reverse_rasterize_to_bsplat_kernel<CDIM, float>,
    ...
    #define __REVERSE_INS__(CDIM)                                               \
        template void launch_reverse_rasterize_to_bsplats_kernel<CDIM>(  \
            const at::Tensor&,                                                    \
            const at::Tensor&,                                                    \
            const at::Tensor&,                                                    \
            const at::Tensor&,                                                    \
