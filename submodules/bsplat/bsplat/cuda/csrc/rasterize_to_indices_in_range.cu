...
__global__ void rasterize_to_indices_in_range_kernel(
    ...
    const T *__restrict__ betas,     // [C, N]
    ...
    int64_t *__restrict__ primitive_ids,       // [n_elems]
    ...
    for (uint32_t b = range_start; b < min(range_end, num_batches); ++b) {
        ...
        for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
            ...
            const vec4<T> xy_opac = xy_opacity_batch[t];
            const T opac = xy_opac.z;
            const T beta = xy_opac.w;
            const vec2<T> delta = {xy_opac.x - px, xy_opac.y - py};
            const T sigma = (conic.x * delta.x * delta.x +
            ...
            T alpha = min(0.999f, opac * __powf(1.f - sigma, beta));
...
std::tuple<torch::Tensor, torch::Tensor> rasterize_to_indices_in_range_tensor(
    ...
    return std::make_tuple(primitive_ids, pixel_ids);
}
